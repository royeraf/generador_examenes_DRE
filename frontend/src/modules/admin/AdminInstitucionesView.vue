...
<template>
  <div class="min-h-screen bg-surface font-sans relative flex flex-col overflow-x-hidden">
    ...
    <div class="max-w-7xl mx-auto w-full relative z-10 flex-1 flex flex-col p-4 sm:p-8">
      ...
      <!-- Main Action Bar -->
      <div class="flex flex-col lg:flex-row items-center justify-between gap-4 mb-8 bg-surface-card p-6 rounded-2xl border-2 border-slate-200 dark:border-slate-700 shadow-sm animate-in fade-in slide-in-from-top-2 duration-500">
        <div class="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
          <!-- Búsqueda -->
          <div class="relative w-full sm:w-80">
            <Search class="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400" />
            <input v-model="searchQuery" type="text" placeholder="Buscar por nombre o código..."
              class="w-full pl-12 pr-4 py-2.5 bg-surface-input border border-slate-300 dark:border-slate-600 rounded-xl text-sm font-bold text-slate-700 dark:text-text outline-none focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500 transition-all" />
          </div>
          <!-- Filtro provincia -->
          <div class="relative w-full sm:w-56">
            ...
            <select v-model="filtroProvinciaId"
              class="w-full pl-10 pr-8 py-2.5 bg-surface-input border border-slate-300 dark:border-slate-600 rounded-xl text-sm font-bold text-slate-700 dark:text-text outline-none focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500 transition-all appearance-none cursor-pointer">
      ...
      <div v-else class="flex-1 flex flex-col">
            ...
            <table class="w-full text-left border-collapse text-sm">
              ...
              <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                <tr v-for="ie in paginadas" :key="ie.id" class="hover:bg-slate-50 dark:hover:bg-slate-900/50 transition-colors group">
                  <td class="p-5 font-mono font-black text-emerald-600 dark:text-primary">{{ ie.codigo_modular }}</td>
                  <td class="p-5">
                    <div class="font-black text-text text-base tracking-tight">{{ ie.nombre }}</div>
                  ...
                  <td class="p-5">
                    <span class="inline-flex items-center gap-2 px-3 py-1 bg-surface-input rounded-full text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-text-muted">
                ...
                <tr v-if="filtradas.length === 0">
                  <td colspan="6" class="p-20 text-center">
                    <div class="flex flex-col items-center">
                      <Building2 class="w-16 h-16 text-slate-200 dark:text-text-subtle mb-4" />
          ...
          <div class="px-6 py-4 bg-surface-input/30 border-t border-slate-200 dark:border-slate-700 flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-4 flex-wrap">
              <p class="text-xs text-slate-500 dark:text-text-muted whitespace-nowrap">
                Mostrando
                <span class="font-bold text-slate-700 dark:text-text">{{ filtradas.length === 0 ? 0 : (currentPage - 1) * pageSize + 1 }}</span>
                –
                <span class="font-bold text-slate-700 dark:text-text">{{ Math.min(currentPage * pageSize, filtradas.length) }}</span>
                de
                <span class="font-bold text-slate-700 dark:text-text">{{ filtradas.length }}</span>
              </p>
                ...
                <button v-for="n in PAGE_SIZE_OPTIONS" :key="n" @click="pageSize = n"
                  :class="['h-7 px-2.5 rounded-lg text-xs font-bold transition-all cursor-pointer',
                    pageSize === n
                      ...
                      : 'bg-surface-card border border-slate-300 dark:border-slate-600 text-slate-500 dark:text-text-muted hover:border-emerald-400 hover:text-emerald-600']">
                  {{ n }}
                </button>
              </div>
            </div>
        ...
        <div v-else class="space-y-4 pb-20">
          <div v-for="ie in paginadas" :key="ie.id"
            ...
            <div class="flex justify-between items-start mb-4">
              <div class="space-y-1">
                ...
                <h3 class="font-black text-text tracking-tight text-xl leading-tight">{{ ie.nombre }}</h3>
              </div>
              <div class="flex gap-2">
                <button @click="openEdit(ie)" class="p-3 bg-surface-input rounded-2xl text-slate-400 transition-all active:scale-95 cursor-pointer"><Edit2 class="w-5 h-5" /></button>
          ...
          <div v-if="filtradas.length === 0" class="py-20 text-center">
             <Building2 class="w-16 h-16 text-slate-200 dark:text-text-subtle mx-auto mb-4" />
          ...
          <div class="bg-surface-card rounded-2xl border-2 border-slate-200 dark:border-slate-700 p-4 flex flex-col items-center gap-3">
            ...
            <p class="text-[11px] text-slate-500 dark:text-text-muted">
              ...
              · <span class="font-bold text-slate-700 dark:text-text">{{ filtradas.length }}</span> instituciones
            </p>
            ...
            <div class="p-8 space-y-6 max-h-[70vh] overflow-y-auto custom-scrollbar">
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div class="space-y-2">
                  ...
                  <input v-model="form.codigo_modular" type="text" placeholder="7 dígitos" class="w-full bg-surface-input border border-slate-300 dark:border-slate-600 rounded-xl py-2.5 px-3.5 text-sm font-bold text-slate-700 dark:text-text outline-none focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500 transition-all" />
                </div>
                <div class="space-y-2">
                  <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Nombre IE</label>
                  <input v-model="form.nombre" type="text" placeholder="Nombre completo" class="w-full bg-surface-input border border-slate-300 dark:border-slate-600 rounded-xl py-2.5 px-3.5 text-sm font-bold text-slate-700 dark:text-text outline-none focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500 transition-all" />
                </div>
              </div>
              ...
              <div class="space-y-3">
                ...
                <div class="flex gap-2 flex-wrap">
                  <button v-for="n in nivelOptions" :key="n.value" @click="toggleNivel(n.value)"
                    :class="[form.nivel_educativo.includes(n.value) ? 'bg-emerald-600 text-white border-emerald-600 shadow-lg shadow-emerald-500/20' : 'bg-surface-card border-slate-200 dark:border-slate-700 text-slate-500']"
              ...
              <div class="space-y-2">
                ...
                <div class="relative">
                  <select v-model="form.ugel_id" class="w-full bg-surface-input border border-slate-300 dark:border-slate-600 rounded-xl py-2.5 px-3.5 text-sm font-bold text-slate-700 dark:text-text outline-none focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500 transition-all appearance-none cursor-pointer">
              ...
              <div class="space-y-2">
                ...
                <input v-model="form.direccion" type="text" placeholder="Dirección física" class="w-full bg-surface-input border border-slate-300 dark:border-slate-600 rounded-xl py-2.5 px-3.5 text-sm font-bold text-slate-700 dark:text-text outline-none focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500 transition-all" />
              </div>
              ...
              <button @click="form.is_active = !form.is_active" class="flex items-center gap-4 p-2 w-fit cursor-pointer">
                ...
                <span class="text-xs font-black text-slate-600 dark:text-text-muted uppercase tracking-widest">Institución Activa</span>
              </button>
            </div>
