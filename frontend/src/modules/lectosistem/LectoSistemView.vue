          ...
          <div v-show="!configCollapsed" class="flex-1 overflow-y-auto custom-scrollbar p-4 space-y-6">
            ...
            <div class="space-y-3">
              ...
              <div class="flex items-center justify-between pt-2">
                ...
                <button v-if="useTextoBase" @click="showTextosModal = true" class="px-2 py-1 bg-slate-100 dark:bg-slate-800/50 hover:bg-slate-200 dark:bg-slate-200 dark:bg-slate-700/50 rounded border border-slate-300 dark:border-slate-700 text-[10px] font-medium text-slate-800 dark:text-white transition-colors cursor-pointer">
          ...
          <div class="bg-white dark:bg-slate-800 rounded-2xl w-full max-w-4xl shadow-2xl border border-slate-300 dark:border-slate-600 flex flex-col max-h-full">
            ...
            <div class="flex-1 overflow-y-auto p-5">
              <div class="space-y-4">
                ...
                <BaseButton v-if="textosBase.length < 4" variant="secondary" block @click="addTexto">
                  <template #icon><Plus class="w-4 h-4" /></template>Agregar otro texto
                </BaseButton>
              </div>
            </div>
            <div class="p-5 border-t border-slate-300 dark:border-slate-600 text-right"><BaseButton variant="primary" size="sm" @click="showTextosModal = false">Aceptar</BaseButton></div>
          </div>
        </div>
      </Transition>
    </Teleport>
        ...
        <div v-if="asignarModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div class="bg-white dark:bg-slate-800 rounded-2xl w-full max-w-md shadow-2xl border border-slate-300 dark:border-slate-600 flex flex-col">
            ...
            <div class="p-5 border-t border-slate-300 dark:border-slate-600 flex justify-end gap-3">
              ...
              <BaseButton variant="primary" size="sm" :disabled="!codigoClaseIdLecto" :loading="loadingAsignar" @click="confirmarAsignar">
                <template #icon><Send class="w-3.5 h-3.5" /></template>Asignar
              </BaseButton>
            </div>
          </div>
