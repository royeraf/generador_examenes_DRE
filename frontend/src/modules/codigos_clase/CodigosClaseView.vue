...
<template>
  ...
  <main class="max-w-4xl mx-auto px-4 py-8 relative z-10">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
      <div>
        <h2 class="text-2xl font-black text-text">Mis Aulas</h2>
        <p class="text-sm text-slate-500 dark:text-text-muted">Gestiona tus aulas y códigos de acceso para estudiantes</p>
      </div>
    ...
    <div v-if="loading" class="flex flex-col items-center justify-center py-24">
      ...
      <p class="mt-4 text-sm font-bold text-slate-400 dark:text-text-subtle tracking-widest uppercase">Cargando Aulas</p>
    </div>

    <!-- Lista vacía -->
    <div v-else-if="codigos.length === 0"
      class="text-center py-20 bg-surface rounded-2xl border border-slate-300 dark:border-slate-800 shadow-sm group">
      <div class="w-20 h-20 bg-slate-50 dark:bg-surface-card rounded-2xl flex items-center justify-center mx-auto mb-6 group-hover:scale-110 transition-transform duration-500">
        <School class="w-10 h-10 text-slate-300 dark:text-text-subtle" />
      </div>
      <p class="text-text font-black text-xl">No tienes aulas creadas</p>
      ...
      <button @click="openCreate" class="mt-8 text-primary font-bold hover:underline cursor-pointer">
        Comenzar ahora →
      </button>
    </div>
    ...
    <div v-else class="space-y-4">
      ...
      <div class="grid grid-cols-1 gap-4 md:hidden">
        <div v-for="c in codigos" :key="c.id" 
          class="bg-surface border border-slate-300 dark:border-slate-800 rounded-2xl p-5 shadow-sm relative overflow-hidden"
          ...
          <div class="flex justify-between items-start mb-4">
            <div>
              ...
              <h3 class="text-xl font-black text-text">Sección {{ c.seccion }}</h3>
            </div>
            <button @click="toggle(c)" class="p-2 bg-slate-50 dark:bg-surface-card rounded-xl transition-colors cursor-pointer">
              <ToggleRight v-if="c.is_active" class="w-7 h-7 text-teal-500" />
              <ToggleLeft v-else class="w-7 h-7 text-slate-400" />
            </button>
          </div>

          <div class="flex items-center gap-3 bg-slate-50 dark:bg-surface-card/50 p-4 rounded-2xl mb-4 border border-slate-300 dark:border-slate-700/50">
            <div class="flex-1">
              <p class="text-[10px] font-bold text-slate-400 uppercase mb-1">Código de Acceso</p>
              <div class="flex items-center gap-2">
                <span class="font-mono font-black text-2xl text-primary tracking-widest">{{ c.codigo }}</span>
          ...
          <div class="flex items-center justify-between pt-2">
            ...
            <div class="flex items-center gap-2">
              <button @click="abrirQR(c)" class="flex items-center gap-2 px-4 py-2 bg-teal-500/10 text-primary font-bold text-xs rounded-xl hover:bg-teal-500 hover:text-white transition-all cursor-pointer">
              ...
              <button @click="toggle(c)"
                ...
                :class="c.is_active ? 'bg-teal-500' : 'bg-slate-300 dark:bg-surface-elevated'">
      ...
      <!-- Desktop View: Table -->
      <div class="hidden md:block bg-surface border border-slate-300 dark:border-slate-800 rounded-2xl overflow-hidden shadow-sm">
        <table class="w-full text-sm">
          <thead>
            <tr class="bg-slate-50 dark:bg-surface-card/50 text-[11px] font-black text-slate-400 uppercase tracking-[0.1em]">
            ...
            <tr v-for="c in codigos" :key="c.id" class="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-all duration-200" :class="{ 'opacity-60': !c.is_active }">
              <td class="px-6 py-4">
                <div class="flex items-center gap-3">
                  <span class="font-mono font-black text-lg text-primary tracking-widest">{{ c.codigo }}</span>
                  <button @click="copiar(c.codigo)" class="p-1.5 bg-slate-100 dark:bg-surface-card rounded-lg text-slate-400 hover:text-teal-500 transition-all group cursor-pointer">
              ...
              <td class="px-6 py-4">
                <div class="flex flex-col">
                  <span class="font-black text-text">{{ c.grado_nombre || `Grado ${c.grado_id}` }}</span>
                  <span class="text-xs font-bold text-slate-400 uppercase">Sección {{ c.seccion }}</span>
                </div>
              </td>
              <td class="px-6 py-4 text-center">
                <div class="inline-flex items-center px-3 py-1 bg-slate-100 dark:bg-surface-card rounded-full">
                  <span class="text-sm font-black text-slate-700 dark:text-text">{{ c.total_estudiantes }}</span>
              ...
              <td class="px-6 py-4 text-slate-500 dark:text-text-muted font-medium">{{ formatFecha(c.fecha_creacion) }}</td>
              <td class="px-6 py-4">
                <div class="flex items-center justify-end gap-2">
                  <button @click="abrirQR(c)" class="p-2.5 bg-teal-50 dark:bg-emerald-500/10 text-primary rounded-xl hover:bg-teal-500 hover:text-white transition-all shadow-sm cursor-pointer" title="Ver QR">
  ...
  <Teleport to="body">
    <Transition name="qr-modal">
      <div v-if="qrModal"
        ...
        <!-- Card -->
        <div class="w-full max-w-2xl bg-surface rounded-2xl border border-slate-200 dark:border-slate-800 shadow-2xl overflow-hidden">
          ...
          <div class="flex flex-col sm:flex-row items-center sm:items-stretch gap-8 px-8 pb-10 pt-2">
            ...
            <div class="flex-1 flex flex-col justify-center min-w-0">
              ...
              <h2 class="text-2xl font-black text-text mb-7 leading-tight">
                Escanea para unirte
              </h2>
              ...
              <div class="space-y-4 mb-7">
                <div class="flex items-start gap-3">
                  <span class="w-6 h-6 rounded-full border-2 border-slate-400 dark:border-slate-500 flex items-center justify-center shrink-0 mt-0.5 text-[11px] font-bold text-slate-500 dark:text-text-muted">1</span>
                  <p class="text-sm text-slate-600 dark:text-text-muted leading-relaxed">Abre la cámara de tu dispositivo</p>
                </div>
                <div class="flex items-start gap-3">
                  <span class="w-6 h-6 rounded-full border-2 border-slate-400 dark:border-slate-500 flex items-center justify-center shrink-0 mt-0.5 text-[11px] font-bold text-slate-500 dark:text-text-muted">2</span>
                  <p class="text-sm text-slate-600 dark:text-text-muted leading-relaxed">Apunta al código QR de la pantalla</p>
                </div>
                <div class="flex items-start gap-3">
                  <span class="w-6 h-6 rounded-full border-2 border-slate-400 dark:border-slate-500 flex items-center justify-center shrink-0 mt-0.5 text-[11px] font-bold text-slate-500 dark:text-text-muted">3</span>
                  <p class="text-sm text-slate-600 dark:text-text-muted leading-relaxed">Completa el registro con tus datos en el formulario</p>
                </div>
              </div>

              <div class="h-px bg-slate-200 dark:bg-surface-input mb-6" />

              <!-- Info del aula -->
              <p class="text-xs font-bold text-slate-400 dark:text-text-subtle uppercase tracking-widest mb-1">
                {{ qrModal.grado_nombre }} — Sección {{ qrModal.seccion }}
              </p>
              <div class="flex items-center gap-3 mb-3">
                <span class="font-mono font-black text-3xl text-primary tracking-[0.2em]">{{ qrModal.codigo }}</span>
                <button @click="copiar(qrModal.codigo)"
                  class="p-2 bg-slate-100 dark:bg-surface-card rounded-xl text-slate-400 hover:text-teal-500 transition-all cursor-pointer">
                  <Copy class="w-4 h-4" />
                </button>
              </div>
              <p class="text-[11px] text-primary font-mono break-all mb-7 leading-relaxed select-all">
                {{ registroUrl(qrModal.codigo) }}
              </p>

              <!-- Botón descargar -->
              <button @click="descargarQR" :disabled="!qrDataUrl"
                class="self-start flex items-center gap-2 px-5 py-2.5 bg-slate-900 dark:bg-white text-white dark:text-text-inverse font-bold text-sm rounded-xl shadow transition-all hover:-translate-y-0.5 active:scale-95 disabled:opacity-40 cursor-pointer">
            ...
            <!-- Separador vertical -->
            <div class="hidden sm:block w-px bg-slate-200 dark:bg-surface-input shrink-0" />

            <!-- Derecha: QR -->
            <div class="flex items-center justify-center shrink-0">
              <div v-if="qrLoading"
                class="w-52 h-52 flex items-center justify-center bg-slate-50 dark:bg-surface-card rounded-2xl animate-pulse">
  ...
  <Teleport to="body">
    <Transition name="details-modal">
      <div v-if="showModal" class="fixed inset-0 z-[110] flex items-end sm:items-center justify-center sm:p-4">
        ...
        <div class="relative bg-surface w-full sm:max-w-md max-h-[92dvh] sm:max-h-fit flex flex-col sm:rounded-2xl rounded-t-2xl shadow-2xl overflow-hidden z-10 animate-slide-up">
          ...
          <div class="px-8 py-6 border-b border-slate-300 dark:border-slate-800 flex items-center justify-between">
            <h2 class="text-xl font-black text-text tracking-tight">Nueva Aula</h2>
          ...
          <div class="p-8 space-y-6 overflow-y-auto">
            ...
            <div>
              ...
              <select v-model="form.grado_id"
                class="w-full bg-surface-input border border-slate-300 dark:border-slate-600 rounded-xl py-2.5 px-3.5 text-sm font-bold text-text outline-none transition-all appearance-none cursor-pointer">
            ...
            <div>
              ...
              <div class="relative">
                 <input v-model.number="form.max_estudiantes" type="number" min="1" max="200"
                  class="w-full bg-surface-input border border-slate-300 dark:border-slate-600 rounded-xl py-2.5 px-3.5 text-sm font-bold text-text outline-none focus:ring-2 focus:ring-teal-500/50 focus:border-teal-500 transition-all" />
                 <span class="absolute right-5 top-1/2 -translate-y-1/2 text-xs font-black text-slate-400 uppercase">Capacidad</span>
              </div>
            </div>
          </div>

          <div class="px-8 py-6 bg-slate-50 dark:bg-surface-card/50 border-t border-slate-300 dark:border-slate-800 flex flex-col sm:flex-row gap-3">
            <button @click="showModal = false" class="flex-1 px-6 py-4 text-sm font-bold text-slate-500 dark:text-text-muted hover:bg-slate-100 dark:hover:bg-slate-700 rounded-2xl transition-colors cursor-pointer">
              Cancelar
            </button>
