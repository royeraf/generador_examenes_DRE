...
<template>
    <div class="flex flex-col order-2 lg:order-2">
        ...
        <!-- Loading State -->
        <div v-if="loading"
            class="h-[400px] sm:h-[580px] lg:h-[650px] bg-gradient-to-br from-white via-indigo-50 to-purple-50 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 rounded-xl border border-indigo-200 dark:border-slate-700/50 text-center flex flex-col items-center justify-center shadow-lg p-6 relative overflow-hidden">
            <div class="absolute inset-0 opacity-20">
                <div class="absolute top-1/4 left-1/4 w-32 h-32 bg-indigo-500/30 rounded-full blur-3xl animate-pulse">
                </div>
                <div class="absolute bottom-1/3 right-1/4 w-40 h-40 bg-purple-500/20 rounded-full blur-3xl animate-pulse"
                ...
                <div class="flex flex-col sm:flex-row items-center justify-between gap-3">
                    ...
                    <div class="flex gap-2 w-full sm:w-auto">
                        <button @click="emit('vincularSistematizador')"
                            class="flex-1 sm:flex-initial px-4 py-2 bg-white/90 text-amber-600 hover:bg-amber-50 text-xs sm:text-sm font-bold rounded-xl transition-all duration-300 flex items-center justify-center gap-2 shadow-lg hover:-translate-y-0.5">
                        ...
                        <button @click="emit('descargarExamenWord')" :disabled="descargandoWord"
                            class="flex-1 sm:flex-initial px-4 py-2 bg-white text-indigo-600 hover:bg-indigo-50 text-xs sm:text-sm font-bold rounded-xl transition-all duration-300 flex items-center justify-center gap-2 shadow-lg hover:-translate-y-0.5">
