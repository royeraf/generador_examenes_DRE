  ...
  <header class="sticky top-0 z-50 h-14 flex items-center justify-between px-4 sm:px-6 gap-3
    ...
    <div class="flex items-center gap-3 shrink-0">
      <div class="relative w-8 h-8 shrink-0">
        ...
        <div class="absolute inset-0 flex items-center justify-center p-1.5 bg-white dark:bg-slate-800
          rounded-lg border border-slate-100 dark:border-slate-700 overflow-hidden">
    ...
    <div class="flex items-center gap-2 shrink-0">
      ...
      <button v-if="showHome" @click="router.push('/')"
        class="p-2 sm:px-3 sm:py-1.5 rounded-full bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700
          border border-slate-200 dark:border-slate-700 transition-colors
          text-xs font-medium text-slate-600 dark:text-slate-300 flex items-center gap-1.5"
        title="Inicio">
